...
public class User extends Endpoint {
	...
	public void handlePost(HttpExchange r) throws IOException, JSONException {
		...
		if (splitUrl[2].equals("register"))
		{
			...
			// See if user is already registered with that email, return bad request
			// make query and get required data, return 500 if error
			ResultSet rs;
			boolean resultHasNext;
			try {
			...
			// check if user was found, return 400 if found
			if (resultHasNext) {
				this.sendStatus(r, 400);
				return;
			}

			// user is not already registered, register user and return ok
			try {
				this.dao.registerUser(email, password, name);
			}
			catch (SQLException e) {
				this.sendStatus(r, 500);
				return;
		...
		// Handle login
		else if (splitUrl[2].equals("login")) {
			String body = Utils.convert(r.getRequestBody());
			JSONObject deserialized = new JSONObject(body);
			...
			// check if we have the correct password, return 403 if incorrect
			try {
